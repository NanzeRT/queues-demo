...
impl<T> Index<T> {
  ...
  pub fn to_bytes(&self) -> [u8; 16] {
    ...
    let mut bytes = [0; 16];
    let index_bytes = (self.index.get() as u64).to_le_bytes();
    let generation_bytes = self.generation.to_le_bytes();
    bytes[..8].copy_from_slice(&index_bytes);
    bytes[8..].copy_from_slice(&generation_bytes);
    bytes
  }
  ...
  pub fn from_bytes(bytes: [u8; 16]) -> Self {
    const _: () = assert!(size_of::<usize>() <= 8);
    let index =
      NonMaxUsize::new(u64::from_le_bytes(bytes[..8].try_into().unwrap()) as usize).unwrap();
